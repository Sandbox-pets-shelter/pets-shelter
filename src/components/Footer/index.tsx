      ...
      <div className={s.upblock}>
        <div className={s.left}>
          ...
          <div className={s.left__icons}>
            {iconsFooter.map((icon) => (
              <div key={icon.id} className={s.left__social}>
                <img className={s.left__icon} src={icon.src} alt="icon" />
              </div>
            ))}
          </div>
        </div>
        <div className={s.right}>
          {footerTextRight.map((item: IFooterText) => (
            <div className={s[item.class]} key={item.id}>
              {item.title && (
                <h2 className={s.right__title} key={item.id}>
                  {item.title}
                </h2>
              )}
              <ul
              ...
              >
                {item.subtitleLinks.map((link: ILink) => (
                  <a key={link.id} className={s.right__link}>
                    {link.text ? link.text : <img src={link.src} className={s.right__links__icon} alt="icon" />}
                  </a>
                ))}
              </ul>
            </div>
