...
const InputField = ({ onChange, type, name, label, placeholder, value = '', disabled, required }: Props) => {
  ...
  const handlePhoneKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (
      event.key === 'Backspace' &&
      getNumbersInputValue(input.value).length === 1
    ) {
      setTelValue('')
    }
  }
      ...
      <div>
        ...
        <div className={`${s.wrapper} ${value !== '' && s.active} ${disabled && s.disabled}`}>
          <input
            ...
            value={value}
          />
          {type === 'password' && <EyeIcon onClick={() => setPasswordShown(!passwordShown)} passwordShown={passwordShown} className={s.icon} active={value !== '' && true}/>}
        </div>
      </div>
    )
  }
    ...
    return (
      <div className={s.num_wrapper}>
        ...
        <div className={`${s.wrapper} ${s.input_num} ${value !== '' && s.active} ${disabled && s.disabled}`}>
          <div className={`${s.button} ${disabled && s.button_disabled}`} onClick={() => onChange((`${parseInt(value) - 1}`).slice(0, 4))}><span className={`arrow arrow-active down ${disabled && 'arrow-disabled'}`}></span></div>
          <input
            ...
            value={value}
          />
          <div className={`${s.button} ${disabled && s.button_disabled} ${s.plus}`} onClick={() => onChange((`${parseInt(value) + 1}`).slice(0, 4))}><span className={`arrow arrow-active up ${disabled && 'arrow-disabled'}`}></span></div>
        </div>
      </div>
    )
  }
